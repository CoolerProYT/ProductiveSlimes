...
public class ModRecipeProvider extends RecipeProvider implements IConditionBuilder {
    ...
    protected void buildRecipes(Consumer<FinishedRecipe> recipeOutput) {
        ...
        for(Tier tier : Tier.values()){
            ...
            //Soliding Recipe
            solidingRecipe(recipeOutput, ModTierLists.getBucketItemByName(name).get(), ModTierLists.getItemByKey(modTiers.growthItemKey()), 1, modTiers.solidingOutputAmount());
    ...
    private static void smeltingRecipe(Consumer<FinishedRecipe> pFinishedRecipeConsumer, ItemLike pIngredient, ItemLike pResult, float pExperience, int pCookingTime) {
        SimpleCookingRecipeBuilder.smelting(Ingredient.of(pIngredient), RecipeCategory.BUILDING_BLOCKS, pResult, pExperience, pCookingTime).unlockedBy(getHasName(pIngredient), has(pIngredient)).save(pFinishedRecipeConsumer);
    }
}
