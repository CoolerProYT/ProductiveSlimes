...
public class GuideBookScreenHelper {
    ...
    public static SlimeData generateSlimeData(ModTiers tiers) {
        return new SlimeData(
                ...
                tiers.color(),
                tiers.cooldown(),
                BuiltInRegistries.ITEM.get(new ResourceLocation(ProductiveSlimes.MODID, tiers.name() + "_slimeball")).getDefaultInstance(),
                ModTierLists.getItemByKey(tiers.growthItemKey()).asItem().getDefaultInstance(),
                (EntityType<BaseSlime>) BuiltInRegistries.ENTITY_TYPE.get(new ResourceLocation(ProductiveSlimes.MODID, tiers.name() + "_slime"))
        );
    }
}