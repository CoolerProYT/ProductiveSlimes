...
public class EndStoneSlime extends BaseSlime{
    public EndStoneSlime(EntityType<? extends Slime> pEntityType, Level pLevel) {
        ...
        if(pHand == InteractionHand.MAIN_HAND) {
            if(pPlayer.isCrouching()) {
                if(!level().isClientSide){
                    ...
                    if (pPlayer.getItemInHand(pHand).getItem() == Items.END_STONE && this.getSize() < 4 && pPlayer.getItemInHand(pHand).getCount() > this.getSize()) {
    ...
    protected ParticleOptions getParticleType() {
        return new ItemParticleOption(ParticleTypes.ITEM, new ItemStack(Items.END_STONE));
    }
}
