...
public class PrismarineSlime extends BaseSlime{
    public PrismarineSlime(EntityType<? extends Slime> pEntityType, Level pLevel) {
        super(pEntityType, pLevel, 1500);
    }

    @Override
    public void dropResource() {
        ItemEntity itemEntity = new ItemEntity(this.level(), this.getX(), this.getY(), this.getZ(), new ItemStack(ModItems.PRISMARINE_SLIME_BALL.get(), this.getSize()));
        ...
        if(pHand == InteractionHand.MAIN_HAND) {
            if(pPlayer.isCrouching()) {
                if(!level().isClientSide){
                    ...
                    if (pPlayer.getItemInHand(pHand).getItem() == Items.TUFF && this.getSize() < 4 && pPlayer.getItemInHand(pHand).getCount() > this.getSize()) {
