...
public class RedstoneSlime extends BaseSlime{
    public RedstoneSlime(EntityType<? extends Slime> pEntityType, Level pLevel) {
        super(pEntityType, pLevel, 7000);
    }

    @Override
    public void dropResource() {
        ItemEntity itemEntity = new ItemEntity(this.level(), this.getX(), this.getY(), this.getZ(), new ItemStack(ModItems.REDSTONE_SLIME_BALL.get(), this.getSize()));
        ...
        if(pHand == InteractionHand.MAIN_HAND) {
            if(pPlayer.isCrouching()) {
                if(!level().isClientSide){
                    ...
                    if (pPlayer.getItemInHand(pHand).getItem() == Items.REDSTONE_BLOCK && this.getSize() < 4 && pPlayer.getItemInHand(pHand).getCount() > this.getSize()) {
    ...
    protected ParticleOptions getParticleType() {
        return new ItemParticleOption(ParticleTypes.ITEM, new ItemStack(ModItems.REDSTONE_SLIME_BALL.get()));
    }
}
