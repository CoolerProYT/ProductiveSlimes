...
public class SolidingStationBlock extends Block implements TranslucentHighlightFix {
    ...
    public void onRemove(BlockState pState, World pLevel, BlockPos pPos, BlockState pNewState, boolean pMovedByPiston) {
        if (pState.getBlock() != pNewState.getBlock()){
            ...
            if (blockEntity instanceof SolidingStationBlockEntity){
                ...
                solidingStationBlockEntity.drops();
    ...
    public ActionResultType use(BlockState pState, World pLevel, BlockPos pPos, PlayerEntity pPlayer, Hand pHand, BlockRayTraceResult pHit) {
        if (!pLevel.isClientSide()) {
            ...
            if (entity instanceof SolidingStationBlockEntity) {
                NetworkHooks.openGui(((ServerPlayerEntity)pPlayer), (SolidingStationBlockEntity)entity, pPos);
            } else {
    ...
    public void setPlacedBy(World pLevel, BlockPos pPos, BlockState pState, @Nullable LivingEntity pPlacer, ItemStack pStack) {
        ...
        if (be instanceof SolidingStationBlockEntity) {
            SolidingStationBlockEntity solidingStationBlockEntity = (SolidingStationBlockEntity) be;
