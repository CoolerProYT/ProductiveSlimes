...
import com.coolerpromc.productiveslimes.block.entity.renderstate.FluidTankBlockEntityRenderState;
...
public class FluidTankBlockEntityRenderer implements BlockEntityRenderer<FluidTankBlockEntity, FluidTankBlockEntityRenderState> {
    ...
    @Override
    public void extractRenderState(FluidTankBlockEntity blockEntity, FluidTankBlockEntityRenderState renderState, float p_446851_, Vec3 p_445788_, @Nullable ModelFeatureRenderer.CrumblingOverlay p_446944_) {
        BlockEntityRenderer.super.extractRenderState(blockEntity, renderState, p_446851_, p_445788_, p_446944_);
        renderState.fluidTankBlockEntity = blockEntity;
    }

    @Override
    public void submit(FluidTankBlockEntityRenderState renderState, PoseStack poseStack, SubmitNodeCollector nodeCollector, CameraRenderState p_451022_) {
        FluidStack fluidStack = renderState.fluidTankBlockEntity.getFluidStack();
        renderFluid(poseStack, nodeCollector, renderState.lightCoords, OverlayTexture.NO_OVERLAY, fluidStack);
    }

    public static void renderFluid(PoseStack pPoseStack, SubmitNodeCollector nodeCollector, int pPackedLight, int packedOverlay, FluidStack fluidStack){
        ...
        nodeCollector.submitCustomGeometry(pPoseStack, RenderTypes.entityTranslucent(sprite.atlasLocation()), (pose, builder) -> {
            ...
            drawQuad(builder, pose, 0.8f, 0.05f, 0.80f, 0.2f, finalHeight, 0.80f, sprite.getU0(), sprite.getV0(), sprite.getU1(), sprite.getV1(), pPackedLight, color, 0, 0, 1);
            pPoseStack.popPose();
            pPoseStack.pushPose();
            pPoseStack.mulPose(Axis.YP.rotationDegrees(90));
            pPoseStack.translate(-1f, 0, 0);
            drawQuad(builder, pose, 0.2f, 0.05f, 0.2f, 0.20f, finalHeight, 0.8f, sprite.getU0(), sprite.getV0(), sprite.getU1(), sprite.getV1(), pPackedLight, color, -1, 0, 0);
            pPoseStack.popPose();
            pPoseStack.pushPose();
            pPoseStack.mulPose(Axis.YN.rotationDegrees(90));
            pPoseStack.translate(0, 0, -1f);
            drawQuad(builder, pose, 0.8f, 0.05f, 0.2f, 0.80f, finalHeight, 0.8f, sprite.getU0(), sprite.getV0(), sprite.getU1(), sprite.getV1(), pPackedLight, color, 1, 0, 0);
            pPoseStack.popPose();
        });
    }
