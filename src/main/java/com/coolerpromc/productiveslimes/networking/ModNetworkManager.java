    ...
    private static Set<IEnergyStorage> findAllConsumersForNetwork(ServerLevel world, CableNetwork network) {
        ...
        for (BlockPos cablePos : network.getCablePositions()) {
            for (Direction dir : Direction.values()) {
                ...
                LazyOptional<IEnergyStorage> maybeStorage = neighbor.getCapability(ForgeCapabilities.ENERGY);

                if (maybeStorage.resolve().isPresent() && maybeStorage.resolve().get().canReceive()) {
                    consumers.add(maybeStorage.resolve().get());
                }
            }
        }
        return consumers;
